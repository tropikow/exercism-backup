...
export function translate2d(dx, dy) {  
  return function executeTranslate(x,y) {
    return [x+dx, y+dy]
  }  
}
...
export function scale2d(sx, sy) {  
  return function executeScale(x,y) {    
    return [x*sx, y*sy]
  }
}
...
export function composeTransform(f, g) {      
  return function executeCompose(x,y) {        
    const [translateX,translateY] = f(x,y)
    return g(translateX,translateY)
  }
}
...
export function memoizeTransform(f) {
  let cache = {}
  return function executeMemoize(x,y) {
    const stringKey = `${x},${y}`
    if(!cache[stringKey]) {
      cache = {}
      cache[stringKey] = f(x,y)
    }        
